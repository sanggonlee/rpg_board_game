# Reference: http://www.pygame.org/project-EzText-920-.html
...
class Input:
    ...
    def get_size(self):
        return self.font.size(self.prompt)

    def get_centre_surface(self, screen):
        centre = screen.subsurface(
            ((screen.get_width()-self.get_size()[0])/2,
             (screen.get_height()-self.get_size()[1])/2),
            self.get_size()
        )
        return centre
        ...
        for event in events:
            ...
            if event.type == KEYDOWN:
                ...
                if not self.shifted:
                    ...
                    elif event.key == K_RETURN: self.return_pressed = True
                elif self.shifted:
                    if event.key == K_a and 'A' in self.restricted: self.value += 'A'
                    elif event.key == K_b and 'B' in self.restricted: self.value += 'B'
                    elif event.key == K_c and 'C' in self.restricted: self.value += 'C'
